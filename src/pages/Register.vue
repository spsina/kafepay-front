      ...
      <form class="ui large form">
        <div class="ui stacked segment">
          ...
          <div class="field">
            <div class="ui left icon input">
              <i class="lock icon"></i>
              <input type="password" name="password1" v-model="user.password1" placeholder="Password">
            </div>
          </div>
          <div class="field">
            <div class="ui left icon input">
              <i class="lock icon"></i>
              <input type="password" name="password2" v-model="user.password2" placeholder="Confirm Password">
            </div>
          </div>
          <!-- Timezone
          <select class="form-control" v-model="user.timezone" name="timezone">
    ...
    <!-- <p style="margin-top:8px"></p>
          ...
          <!-- <div class="field" style="text-align:left; margin-top:10px;">
            <div class="ui checkbox">
                <input type="checkbox" name="student" v-model="user.is_student" />
          ...
          <div class="field">
            <div class="ui left icon input">
              ...
              <input type="text" v-model="user.university" placeholder="Institute or Company" />
            </div>
          </div>
      ...
      <div class="ui message">
        Already registered? <router-link :to="{name: 'login'}" tag="a">Login</router-link>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    submit() {
      ...
      var vinst = this;
      axios
        .post(this.$store.state.hostUrl + '/api/v2/auth/registration/',
        this.user,
        {
          headers: {
        ...
        .then(response => {
          ...
          axios
            .post(this.$store.state.hostUrl + '/api/v2/profile/update/',
            vinst.user,
            {
              headers: {
                "Content-type": "application/json",
                Authorization: "Token " + vinst.$store.state.token
              }
            }).then(response => {
              if (vinst.$route.query.next) {
                vinst.$router.push(vinst.$route.query.next);
              } else {
              ...
              console.log(error);
              vinst.$store.commit("removeToken");
              vinst.loading = false;
            });
          vinst.loading = false;
        })
